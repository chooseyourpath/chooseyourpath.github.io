# Purpose: generate a list of readers from the current /Downloads directory
#
# 1. get a list of all the files in the format customer.book.reader.recordingPostionID.webm
# 1.a. result= list of files in format "../customer.17135.teacher2.1_1_a.webm"
#
...
# 
# 6. Redirect the result into a allRecorders.txt file
ls ../*.*.*.*.webm | sed 's/\.\.\///; s/[^.]*.//;s/[^.]*.//;s/\..*//' | sort | uniq | sed '/^\s*$/d' > allRecordersGenerated.txt
